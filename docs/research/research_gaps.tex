...
\DeclareNotationKey{status}{%
  ...
  Checks V1--V12 come from \texttt{sims/verify\_theory.m}: evidence, never proof.
...
\DeclareNotationKey{valid}{%
  Checks V1--V12 come from \texttt{sims/verify\_theory.m}; gap runs come from \texttt{sims/research.m}.
...
Checks V1 to V12, referenced throughout the preceding sections, are produced by \texttt{sims/verify\_theory.m}. Each is a falsification attempt: it plots a measured quantity against the quantity the theory predicts and records a verdict in \texttt{sims/outputs/verify\_theory/summary.json}. Regenerate everything with
\begin{verbatim}
cd sims && matlab -batch "verify_theory"
\end{verbatim}
